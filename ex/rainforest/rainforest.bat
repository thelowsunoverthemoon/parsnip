...
%PARSNIP_MAKE% music[rain] "tracks\rain.mp3" loop 100
%PARSNIP_MAKE% music[frogs] "tracks\frog.mp3" loop 100
%PARSNIP_MAKE% music[birds] "tracks\bird.mp3" loop 100

%PARSNIP_PLAY: # = music[rain]%
%PARSNIP_PLAY: # = music[frogs]%
%PARSNIP_PLAY: # = music[birds]%

FOR /L %%$ in () DO (
    CLS
    ECHO Rainforest Ambience
    ECHO Select [R] rain
    ECHO        [F] frogs
    ECHO        [B] birds
    CHOICE /C RFB /N
    IF !ERRORLEVEL! EQU 1 (
        SET "music[type]=!music[rain]!"
    ) else IF !ERRORLEVEL! EQU 2 (
        SET "music[type]=!music[frogs]!"
    ) else (
        SET "music[type]=!music[birds]!"
    )
    CHOICE /C CD /N /M "[C] crescendo, [D], decrescendo"
    IF !ERRORLEVEL! EQU 1 (
        %PARSNIP_CRESCENDO: # = music[type], @ = 20, ? = 5%
    ) else (
        %PARSNIP_DECRESCENDO: # = music[type], @ = 20, ? = 5%
    )
)
